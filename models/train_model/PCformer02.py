...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.decembedding = nn.Linear(in_features=self.seq_len,out_features=self.pred_len)
        ...
        self.dec_value_embedding = PatchEmbedding(configs.d_model, self.seg_len, self.seg_len, self.pad_in_len - configs.seq_len, 0)
        self.lenx = nn.Linear(95,96)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        diff = torch.diff(x_enc, n=1, dim=1)
        x_enc = x_enc - means
        ...
        x_dec = self.lenx(diff.permute(0, 2, 1))
        x_dec = self.decembedding(x_dec)
        x_enc = x_enc.permute(0, 2, 1)

        x_enc, n_varsen = self.enc_value_embedding(x_enc)
        x_dec, n_varsde = self.dec_value_embedding(x_dec)
        x_enc = rearrange(x_enc, '(b d) seg_num d_model -> b d seg_num d_model', d = n_varsen)
        x_dec = rearrange(x_dec, '(b d) seg_num d_model -> b d seg_num d_model', d = n_varsde)
