...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (7, 8, 12), 'pool_size': None,
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        img_size = self.to_tuple(img_size)
        patch_size =  self.to_tuple(patch_size)
...
def cmt_xsT(pretrained=False, **kwargs):
    """
    CMT-XS: dim x 0.9, depth x 0.8, input 192
    """
    model_kwargs = dict(
        qkv_bias=True, embed_dims=[52, 104], stem_channel=7, num_heads=[1, 2],
        depths=[2, 1], mlp_ratios=[3.77, 3.77], qk_ratio=1, sr_ratios=[2, 1], **kwargs)
    model = _create_cmt_model(pretrained=pretrained, **model_kwargs)
    return model

model = cmt_xsT(pretrained=False,)

a = torch.randn(32,7,8,12)
