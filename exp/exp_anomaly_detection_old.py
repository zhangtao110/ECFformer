    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(train_loader):
                ...
                # reconstruction
                outputs = self.model(batch_x, None, None, None)
                # pred_data_train = outputs
                # pred_data_train = pred_data_train[:, -self.args.pred_len:, 0:]
                # pred_data_train = pred_data_train.detach().cpu().numpy()
                # # batch_y = batch_y.detach().cpu().numpy()
                # pred_train = pred_data_train
                # preds_train.append(pred_train)
                # criterion
                score = torch.mean(self.anomaly_criterion(batch_x, outputs), dim=-1)
                score = score.detach().cpu().numpy()
                print('xxaa', score.shape)
                attens_energy.append(score)

        # preds_train = np.array(preds_train)
        # print('jj', pred_train.shape)
        # merged_data_train = preds_train.reshape(-1, 8)
        # import pandas as pd
        # df_train = pd.DataFrame(merged_data_train)
        # df_train.to_csv('./dataset/Oil_anomal_detection/' + self.args.model + '_' + 'pred_train_data.csv',
        #           index_label=None)
        # print('wwzz', attens_energy.shape)
        attens_energy = np.concatenate(attens_energy, axis=0).reshape(-1)
        train_energy = np.array(attens_energy)
        print('xxyy', train_energy.shape)
        ...
        for i, (batch_x, batch_y) in enumerate(thre_loader):
            ...
            pred = pred_data
            preds.append(pred)
            # true = batch_x[:, -self.args.pred_len:, -1:]
            # trues.append(true)
            # criterion
            score = torch.mean(self.anomaly_criterion(batch_x, outputs), dim=-1)
            score = score.detach().cpu().numpy()
        ...
        df.to_csv('./dataset/Oil_anomal_detection_1/' + self.args.model + '_' + 'pred_data.csv',
                                index_label=None)
        # print('outputs', merged_data1.shape)
        # # 绘制曲线
        # plt.plot(merged_data1)
        # plt.xlabel('Index')
        # plt.ylabel('Value')
        # plt.title('Tensor Plot')
        #
        # # 保存到本地
        # plt.savefig('merged_data1_plot.png')

        attens_energy = np.concatenate(attens_energy, axis=0).reshape(-1)
        test_energy = np.array(attens_energy)
        print('xx', test_energy.shape)
        combined_energy = np.concatenate([train_energy, test_energy], axis=0)
        threshold = np.percentile(combined_energy, 100 - self.args.anomaly_ratio)
        print('xx', 100 - self.args.anomaly_ratio)
        ## anomaly_ratio
