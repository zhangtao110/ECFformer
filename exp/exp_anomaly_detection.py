...
class Exp_Anomaly_Detection(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        mm = merged_data1[:, 0]
        # print('mm', merged_data1.shape)

        window_size = self.args.seq_len
        stride = 10
        interval = 30
        preds_subsequences_new = [mm[i:i + window_size] for i in range(0, len(mm), window_size)]
        preds_new = preds_subsequences_new[0]
        for subsequence in preds_subsequences_new[1:]:
            preds_new = np.concatenate((preds_new, subsequence[-10:]))

        merged_data1 = preds_new.reshape(-1,1)
        print('xx', merged_data1.shape)
        # print('mm1', merged_data1.shape)
        ...
        train_data = data[::1]
        pre_train_data = train_data[:-interval]
        after_train_data = train_data[interval:]
        process_train_data = after_train_data - pre_train_data
        # 创建一个含有5个零的数组
        zeros_train = np.zeros((interval, process_train_data.shape[1]), dtype=process_train_data.dtype)
        ...
        scaler.fit(process_train_data)
        # scaler.fit(data)
        test_data = pd.read_csv(os.path.join('./dataset/Oil_anomal_detection_1', 'test-17.csv'))
        test_data = test_data.drop(columns=['date'])
        test_data = test_data.values[:, 1:]
        test_data = np.nan_to_num(test_data)
        test_data = test_data[::1]
        # test_data = scaler.transform(test_data)
        pre_test_data = test_data[:-interval]
        after_test_data = test_data[interval:]
        merged_data1 = scaler.inverse_transform(merged_data1)
        # merged_data1 = after_test_data[0:merged_data1.shape[0]] - merged_data1[30:]  ## 14阶段数据
        merged_data1 = after_test_data[0:merged_data1.shape[0]] - merged_data1[27:]  ## 15阶段 17阶段
        print('sbc', merged_data1.shape)
        ...
        with torch.no_grad():
            for batch_x, _, batch_pre_mean in thre_loader:
                ...
                # 扩展每个子序列标签为整个子序列
                expanded_labels = np.repeat(batch_labels, batch_x.size(1))
        ...
        window_size = self.args.seq_len
        stride = 10
        data_res_number = self.args.seq_len - 10
        ...
        for subsequence in pred_subsequences_new[1:]:
            if all(subsequence[-3:]):
                pred_labels_new[-data_res_number:] = [1] * data_res_number
            pred_labels_new = np.concatenate((pred_labels_new, subsequence[-10:]))
        # 拼接所有子序列为一个新的numpy数组
        ...
        for subsequence in test_subsequences_new[1:]:
            test_labels_new = np.concatenate((test_labels_new, subsequence[-10:]))
