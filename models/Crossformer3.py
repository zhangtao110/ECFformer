...
from layers.Crossformer_EncDec import scale_block4, Encoder, Decoder, DecoderLayer, scale_block3rand
from layers.Embed import PatchEmbedding
from layers.SelfAttention_Family import AttentionLayer, FullAttention, TwoStageAttentionLayerMLP
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # self.seg_len = np.random.randint(8,12)
        ...
        # self.out_MLP = nn.Linear(self.seq_len//2//self.seg_len*configs.d_model,self.pred_len)

        # Encoder
        self.encoder = Encoder(
            [
                scale_block3rand(configs, 1 if l is 0 else self.win_size, configs.d_model, configs.n_heads, configs.d_ff,
                            1, configs.dropout,
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    TwoStageAttentionLayerMLP(configs, (self.pad_out_len // self.seg_len), configs.factor, configs.d_model, configs.n_heads,
        ...
        self.final = nn.Conv2d(in_channels=3,out_channels=1,kernel_size=1,padding='same')
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # print('xx', dec_out.shape)
        dec_out = torch.cat(dec_out,dim=1)
        dec_out = self.final(dec_out).squeeze()
        return dec_out + mmm
