...
class Exp_Anomaly_Detection(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        train_data = data[::1]
        pre_train_data = train_data[:-5]
        after_train_data = train_data[5:]
        process_train_data = after_train_data - pre_train_data
        # 创建一个含有5个零的数组
        zeros_train = np.zeros((5, process_train_data.shape[1]), dtype=process_train_data.dtype)
        ...
        # scaler.fit(data)
        test_data = pd.read_csv(os.path.join('./dataset/Oil_anomal_detection_1', 'test.csv'))
        test_data = test_data.drop(columns=['date'])
        test_data = test_data.values[:, 1:]
        test_data = np.nan_to_num(test_data)
        test_data = test_data[::1]
        # test_data = scaler.transform(test_data)
        pre_test_data = test_data[:-5]
        after_test_data = test_data[5:]
        merged_data1 = scaler.inverse_transform(merged_data1)
        merged_data1 = after_test_data[0:merged_data1.shape[0]] - merged_data1
